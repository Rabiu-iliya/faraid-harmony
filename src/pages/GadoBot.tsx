...
export default function GadoBot() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Assalamu Alaikum! I'm GadoBot, your Islamic inheritance assistant. Ask me about Faraid rules, shares, blocking, ʿAwl, Radd, or anything related to inheritance distribution." },
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      if (data?.error) {
        if (data.error.includes("Rate limit")) toast({ title: "Rate limited", description: "Please wait and try again.", variant: "destructive" });
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-8rem)] max-w-3xl mx-auto">
        <h1 className="font-serif text-3xl font-bold text-primary mb-4">GadoBot</h1>
